...
export function NewMeetingContent() {
  ...
  const save = () => {
    ...
    try { (api as any).myCreate('meetings', { id: 'mt' + Date.now(), kind, title: title.trim(), agenda, date, time, duration, location, platform, link, attendees: selectedPersonnel, at: new Date().toISOString() }); } catch (_) {}
    closeModal();
    ...
    <div className="flex flex-col gap-3" style={{ maxHeight: '75vh', overflowY: 'auto' }}>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <FormGroup label="ساعت *">
          <TimePicker value={time} onChange={(v: string) => setTime(v)} />
        </FormGroup>
      </div>

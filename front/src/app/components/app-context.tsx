  ...
  const checkoutCart = useCallback(() => {
    setEuCart(prev => {
      ...
      const newOrders = Object.entries(bySource).map(([source, lines]) => {
        ...
        const _num = fa(1100 + Math.floor(Math.random() * 800));
        ...
        (api as any).placeOrder(source === 'dine' ? 'dine' : 'market', lines, total, { source, num: _num, items: itemsText, vendor: _vendor, eta: _eta, progress: 20, status: 'preparing', date: 'هم‌اکنون', address: (typeof window !== 'undefined' ? ((window as any).__euOrderAddress || '') : ''), ...(_dt ? { tableId: _dt.tableId, tableLabel: _dt.tableLabel, dineType: 'dinein' } : {}) })
        ...
        return {
          ...
          vendor: _vendor,
          date: 'هم‌اکنون', total: fa(total), eta: _eta, progress: 20,
        };
      });

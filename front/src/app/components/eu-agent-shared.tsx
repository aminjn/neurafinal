...
export function AgentTabBar({ tabs, active, onChange, asFooter = false }: { tabs: { id: string; icon: string; label: string; badge?: number }[]; active: string; onChange: (id: string) => void; asFooter?: boolean }) {
  if (asFooter) {
    ...
    return (
      <div className="flex-shrink-0 px-3 glass-footer-mount" style={{ order: 2, paddingBottom: 'max(10px, env(safe-area-inset-bottom))', paddingTop: 4 }}>
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-t border-[var(--aw-border)]">
        <div className="flex-1 flex items-center gap-2 rounded-xl px-3 border border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-input)' }}>
          <input className="flex-1 bg-transparent border-none py-2.5 text-[12px] text-[var(--aw-text-primary)] outline-none placeholder:text-[var(--aw-text-muted)]"
            ...
            onKeyDown={e => { if (e.key === 'Enter' && inputText.trim()) { onOpenFull(); setInputText(''); } }} />
        </div>
        <button className="w-10 h-10 rounded-xl border-none text-white cursor-pointer flex items-center justify-center text-[14px]"
          ...
          onClick={() => { onOpenFull(); if (inputText.trim()) setInputText(''); }}>
...
export function AgentChatTabUI({
  ...
  const handleSend = async () => {
    ...
    const base = [...liveMsgs, { from: 'user' as const, text: txt }];
    setLiveMsgs(base);
    setChatInput('');
    setChatBusy(true);
    try {
      const hist = base.map(m => ({ role: m.from === 'user' ? 'user' : 'assistant', content: m.text }));
      const r: any = await (api as any).chat(uniqueKey, hist);
      const reply = (r && r.ok && r.text) ? String(r.text) : 'الان نتوانستم پاسخ بدهم؛ دوباره تلاش کنید.';
      setLiveMsgs(prev => [...prev, { from: 'agent', text: reply }]);
    } catch (e) {
      setLiveMsgs(prev => [...prev, { from: 'agent', text: 'خطا در ارتباط با دستیار؛ دوباره تلاش کنید.' }]);
    ...
    return (
      <div className="flex-1 flex flex-col min-h-0 relative">
        <div className="flex items-center gap-2.5 px-4 pt-3 pb-2 flex-shrink-0 border-b border-[var(--aw-border)]">
          <button className="w-8 h-8 rounded-[10px] border border-[var(--aw-border)] bg-transparent text-[var(--aw-text-secondary)] cursor-pointer flex items-center justify-center"
        ...
        <AnimatePresence>
          {showTopics && isAgent && (
            ...
            <motion.div className="w-[340px] max-w-[92%] max-h-[82%] flex flex-col rounded-2xl overflow-hidden pointer-events-auto"
              ...
              initial={{ opacity: 0, y: 10, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.96 }} transition={{ duration: 0.2 }}>
